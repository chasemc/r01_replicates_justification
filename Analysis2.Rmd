...
a<-expand.grid(1:3, 1:8)


  xa <- lapply(1:10,function(boot) mapply(theFunc, bio = a$Var2, tech = a$Var1, boot = boot,  SIMPLIFY = FALSE))
...
xa %>% 
    ...
    ggplot() + 
    geom_violin(aes(y=B004, x=factor(techReps))) +
    facet_wrap(~bioReps, scales="fixed")
```

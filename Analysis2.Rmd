...
b <- IDBacApp::createFuzzyVector(massStart = 3000,
                            massEnd = 16000, 
                            ppm = 500, 
                            massList = lapply(unlist(z), function(x)x@mass), 
                            intensityList = lapply(unlist(z), function(x)x@intensity))
colnames(b)<-c("172-6", "A019", "A046", "B004")

...
numCores <- parallel::makeCluster(numCores,useXDR =F) 



 
pp <- parallel::parLapply(numCores, seq(1,999)[which(!1:999 %in% as.integer(tools::file_path_sans_ext(list.files(here::here("localonly",
                                                                                                                            "par")))))], 
                          function(i) {
                            ...
                            results<- lapply(1:48,
                                             function(x){
                                               ...
                                               zz2 <- merge_bio_replicates(z)
                                               
                                               zz2 <- IDBacApp::createFuzzyVector(massStart = 3000,
                                                                                  massEnd = 16000, 
                                                                                  ppm = 500, 
                                                                                  massList = lapply(zz2, function(x)x@mass), 
                                                                                  intensityList = lapply(zz2, function(x)x@intensity))
                                               b <- 1 - coop::cosine(zz2)
